...
//     let image = cv.imread(canvas); // loads the image

//     cv.cvtColor(image, image, cv.COLOR_RGBA2GRAY, 0);
//     cv.threshold(image, image, 175, 255, cv.THRESH_BINARY); // converts all pixels with value above 175 to have a value of 255(white) to give the image more contrast
...
//         heigth = Math.round(image.rows/scaleFactor)
...
//     const LEFT = Math.ceil(4 + (20 - width)/2); // ceil rounds number up
//     const RIGHT = Math.floor(4 + (20 - width)/2); // floor rounds number down
...
//     // Testing only
//     const outputCnavas = document.createElement('CANVAS');
//     cv.imshow(outputCnavas, image);
//     document.body.appendChild(outputCnavas);
